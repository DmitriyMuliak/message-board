...
`vi.mock`, ports/providers and the fixture lifecycle are deliberately out of scope here. Everything below is the code that exists today in
...
| `tests/unit/features/auth/LoginWidgetExample.tsx` | a widget from another team that embeds `LoginForm`                |
| `tests/unit/features/auth/LoginFormTestKit.tsx`   | the kit: props + driver + lifecycle, and a renderer built on it   |
| `tests/unit/features/auth/LoginFormHarness.ts`    | the harness: a world, its MSW handlers, and a scenario vocabulary |
...
```tsx
// tests/unit/features/auth/LoginFormTestKit.tsx
...
```ts
// tests/unit/features/auth/LoginFormHarness.ts
...
`tests/unit/features/auth/LoginForm.test.tsx`:
...
`tests/unit/features/auth/LoginWidgetExample.test.tsx`. `LoginWidgetExample` is "a widget from an
